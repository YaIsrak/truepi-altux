...
export default function ProductTabs() {
  return (
    <Tabs defaultValue="choose" className="w-full">
      <TabsList className="grid w-full grid-cols-2 bg-transparent mb-8 max-w-md justify-center mx-auto">
        <TabsTrigger
          className="text-white text-base data-[state=active]:text-black transition-all px-2 py-5 "
          value="choose"
          asChild
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 text-start gap-12">
        {plans.map((plan, i) => (
          <div key={i}>
            ...
            <Button borderRadius="0.80rem" className="">
              <Image
                src={plan.image}
                alt={plan.name}
                width={600}
        ...
        {plans.map((plan, i) => (
          <div key={i}>
            <div className="divide-y-2">
              <h3
                className="text-xl pb-2 px-6 font-bold"
                style={{ color: plan.color }}
              >
                {plan.name}
              </h3>
              {plan.features.map((feature, i) => (
                <div
                  key={i}
                  className="px-6 h-10 text-center flex justify-center items-center text-xs"
                >
                  {feature}
                </div>
              ))}
